...
def search(request):
    ...
    for cat in cats:
        ...
        if len(prod) != 0:
            print(Product.objects.values('id'))
...
def products(request,myid):
    ...
    print(product)
    return render(request,'shop/productview.html',{'product':product[0],'myid':myid})
...
def checkout(request):
    if request.method == 'POST':
        ...
        order = Order(name = name,email=email,phone=phone,address=address,state=state,city=city,zip_code=zip_code,item_json=item_json,amount=amount)
        order.save()
        ...
        param_dict = {
                ...
                'MID': 'vxARQk44083912398538', #'Your-Merchant-Id-Here',
